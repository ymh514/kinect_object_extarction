...
void initialize(SRM *srm);
void segmentation(SRM *srm);
void finalize(SRM *srm);
void merge_small_regions(SRM *srm);

unsigned int diff(SRM *srm, unsigned int idx1, unsigned int idx2);
unsigned int merge_predicate(SRM *srm, unsigned int reg1, unsigned int reg2);
void bucket_sort(Pair *pairs, Pair *ordered_pairs, unsigned int size);
void merge_regions(SRM *srm, unsigned int r1, unsigned int r2);
...
void DoSRM(double Q, unsigned int width, unsigned int height, unsigned int channels, uint8_t *in, uint8_t *out, unsigned int borders) {
    SRM *srm = srm_new(Q, width, height, channels, borders);
...
SRM* srm_new(double Q, unsigned int width, unsigned int height, unsigned int channels, unsigned int borders) {
    ...
    srm->sizes = (unsigned int*)malloc(srm->size * sizeof(unsigned int));
    srm->pairs = (Pair*)malloc(srm->n_pairs * sizeof(Pair));
    srm->ordered_pairs = (Pair*)malloc(srm->n_pairs * sizeof(Pair));
...
inline unsigned int diff(SRM *srm, unsigned int idx1, unsigned int idx2) {
    unsigned int offset1;
    offset(offset1, idx1, srm->widthStep_in);
    uint8_t r1 = get_r(srm->in, offset1);
    uint8_t g1 = get_g(srm->in, offset1);
    uint8_t b1 = get_b(srm->in, offset1);

    unsigned int offset2;
    offset(offset2, idx2, srm->widthStep_in);
    uint8_t r2 = get_r(srm->in, offset2);
    uint8_t g2 = get_g(srm->in, offset2);
    uint8_t b2 = get_b(srm->in, offset2);
...
// Merge two regions
void merge_regions(SRM *srm, unsigned int reg1, unsigned int reg2) {
    ...
    set_r(srm->out, offset, (uint8_t)r_avg);
    set_g(srm->out, offset, (uint8_t)g_avg);
    set_b(srm->out, offset, (uint8_t)b_avg);
}
